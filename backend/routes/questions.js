...
// // POST to upload a new question (admin)
// router.post('/', async (req, res) => {
//     const { subject, level, newQuestion } = req.body;

//     if (!subject || !level || !newQuestion) {
//         return res.status(400).json({ message: "Subject, level, and question data are required." });
//     }

//     const levelDir = `level${level}`;
//     const subjectPath = path.join(questionsBasePath, subject);
//     const levelPath = path.join(subjectPath, levelDir);
//     const filePath = path.join(levelPath, 'questions.json');

//     try {
//         // Create subject and level directories if they don't exist
//         await fs.mkdir(levelPath, { recursive: true });

//         let questions = [];
//         try {
//             // Try to read existing questions file
//             const data = await fs.readFile(filePath, 'utf8');
//             questions = JSON.parse(data);
//         } catch (error) {
//             // If file doesn't exist, it's okay; we'll create it.
